...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		...
		posY: rangeInput.offset().top + rangeInput.outerHeight(true), // y position
		appendTo: 'body',
		onClose: function(){},
		onOpen: function(){},
		onChange: function(){},
	...
	function showRP(event){
		if(rp.data('state') == 'closed'){ 
			rp.data('state', 'open');
			rp.parent().css('left', jQuery(event.target).offset().left);
			rp.parent().css('top', jQuery(event.target).offset().top+jQuery(event.target).height());
			rp.show();
			options.onOpen(); 
			
		}
	}
	function hideRP(event){
	...
	function toggleRP(event){
		if( rp.data('state') == 'open' ){ hideRP(event); }
		else { showRP(event); }
	}
	rp.data('state', 'closed');
